...
- `ui.getValue` on dropdowns returns **0-based** index — use `options[idx + 1]` to index into a Lua table. `ui.setValue` and the 5th arg to `ui.newDropdown` are both **1-based**. This asymmetry is confirmed: `newDropdown(..., 11)` defaults to the 11th item; `getValue` returns `10`, so `options[10 + 1]` is correct. The user initially believed getValue was 1-based (seeing the 5th arg is 1-based) — it is not. The `+1` offset on all table lookups is correct and required.
- `ui.newDropdown` 5th arg (default) is **1-based** position in the list — passing `0` selects `{None}` (no selection); pass `1` for the first item, `2` for the second, etc.
